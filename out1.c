...
object_t * cubex_main_int()
{
	...
	vtemp_2 = gc_inc(v_input);
	while(iterableHasNext(vtemp_2, (iterableIndex_t *)vtemp_1))
	{
		v_i = gc_inc(iterableNext(vtemp_2, (iterableIndex_t *)vtemp_1));
			vtemp_3 = v_ret;
	v_ret = gc_inc(iterableAppend(iterableAppend(createIterable_value(v_i, 0), v_ret), createIterable_value(v_i, 0)));
	gc(gc_dec(vtemp_3));
	vtemp_3 = NULL;
		gc(gc_dec(v_i));
		v_i = NULL;
